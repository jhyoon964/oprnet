...
class DataAugmentor(object):
    ...
    def photo_metric_distortion(self, data_dict=None, config=None):
        ...
        gt_img = cv2.cvtColor(gt_img, cv2.COLOR_BGR2HSV)
        # random saturation
        if np.random.randint(2):
            rand_saturation = np.random.uniform(saturation_lower, saturation_upper)
            img[..., 1] *= rand_saturation
            gt_img[...,1] *= rand_saturation
        # random hue
        if np.random.randint(2):
            rand_hue = np.random.uniform(-hue_delta, hue_delta)
            img[..., 0] += rand_hue
            img[..., 0][img[..., 0] > 360] -= 360
            img[..., 0][img[..., 0] < 0] += 360
            gt_img[..., 0] += rand_hue
            gt_img[..., 0][gt_img[..., 0] > 360] -= 360
            gt_img[..., 0][gt_img[..., 0] < 0] += 360   
        # convert color from HSV to BGR
        img = cv2.cvtColor(img, cv2.COLOR_HSV2BGR)
        gt_img = cv2.cvtColor(gt_img, cv2.COLOR_HSV2BGR)
        # random contrast
        if mode == 0:
            if np.random.randint(2):
                alpha = np.random.uniform(contrast_lower, contrast_upper)
        ...
        if np.random.randint(2):
            ...
            rand_swap_ch = np.random.permutation(3)
            img = img[..., rand_swap_ch]
            gt_img = gt_img[..., rand_swap_ch]

        data_dict['image'] = img
        data_dict['gt_image'] = gt_img
        return data_dict
