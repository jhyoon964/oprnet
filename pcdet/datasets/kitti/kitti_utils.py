...
def transform_annotations_to_kitti_format(annos, map_name_to_kitti=None, info_with_fakelidar=False):
    ...
    for anno in annos:
        ...
        if len(gt_boxes_lidar) > 0:
            ...
            anno['location'][:, 0] = -gt_boxes_lidar[:, 1] 
            anno['location'][:, 1] = -gt_boxes_lidar[:, 2] 
            anno['location'][:, 2] = gt_boxes_lidar[:, 0]  
            dxdydz = gt_boxes_lidar[:, 3:6]
            anno['dimensions'] = dxdydz[:, [0, 2, 1]] 
...
def calib_to_matricies(calib):
    ...
    """
    V2C = np.vstack((calib.V2C, np.array([0, 0, 0, 1], dtype=np.float32))) 
    R0 = np.hstack((calib.R0, np.zeros((3, 1), dtype=np.float32))) 
    R0 = np.vstack((R0, np.array([0, 0, 0, 1], dtype=np.float32)))
    V2R = R0 @ V2C
    P2 = calib.P2
    return V2R, P2