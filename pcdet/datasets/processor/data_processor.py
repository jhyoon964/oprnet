...
class DataProcessor(object):
    ...
    def imrescale(self, data_dict=None, config=None):
        ...
        img = data_dict['image']
        gt_img = data_dict['gt_image'] # addition
        h, w = img.shape[:2]
        ...
        gt_img = cv2.resize(gt_img, new_size, interpolation=cv2.INTER_LINEAR)
    ...
    def imnormalize(self, data_dict=None, config=None):
        ...
        gt_img = data_dict['gt_image']
        gt_img = gt_img.copy().astype(np.float32)   
        
        mean = np.array(config.MEAN, dtype=np.float64).reshape(1, -1)
        stdinv = 1 / np.array(config.STD, dtype=np.float64).reshape(1, -1)
        if config.TO_RGB:
            cv2.cvtColor(img, cv2.COLOR_BGR2RGB, img)
            cv2.cvtColor(gt_img, cv2.COLOR_BGR2RGB, gt_img)
        cv2.subtract(img, mean, img)  # inplace
        cv2.multiply(img, stdinv, img)  # inplace
        data_dict['image'] = img
        
        cv2.subtract(gt_img, mean, gt_img)  # inplace
        cv2.multiply(gt_img, stdinv, gt_img)  # inplace
        data_dict['gt_image'] = gt_img
        return data_dict
