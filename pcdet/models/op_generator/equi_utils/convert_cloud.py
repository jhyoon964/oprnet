...
import os
import numpy as np
def point_cloud_to_range_image(point_cloud, remissions, isMatrix, return_remission = False, return_points=False):
    if (isMatrix):
        laser_scan = LaserScan()
        laser_scan.set_points(point_cloud, remissions)
...
def range_image_to_point_cloud_fast(range_image):
    ...
    mask = np.logical_and(depth>3.0, depth < 70.4)
    xyz = pts[mask, :]
    return xyz


def convert_image_to_point(median_xyz):
    median_xyz = range_image_to_point_cloud_fast(median_xyz)
    intensity = np.zeros((median_xyz.shape[0], 1), dtype=np.float32)
    output = np.hstack((median_xyz, intensity))
